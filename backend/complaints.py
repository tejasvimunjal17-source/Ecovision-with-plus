...
def update_status(complaint_id, new_status, changed_by, note=""):
    # PHASE 2B: datetime('now') -> now(), PostgreSQL's current-timestamp
    # function (the SQLite-only spelling has no meaning in Postgres). Same
    # UPDATE, same signature, same behavior — three occurrences below.
    execute("UPDATE complaints SET status=?, updated_at=now() WHERE id=?",
            (new_status, complaint_id))
    if new_status == "Resolved":
        execute("UPDATE complaints SET resolved_at=now() WHERE id=?", (complaint_id,))
...
def get_officers():
    # PHASE 2B: unchanged — users.is_active stays INTEGER (0/1) in
    # Postgres (see supabase/migrations/001_init_schema.sql), so this
    # literal needs no translation.
