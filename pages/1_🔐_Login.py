...
with tab_login:
    ...
    # PHASE 3D: replaces the old auto-seeded demo-admin credential hint.
    # Phase 2B removed automatic admin seeding (see
    # supabase/README.md's "Admin account seeding" section) — no
    # admin@ecovision.local / Admin@12345 account exists on a
    # Supabase-backed deployment unless the deployer explicitly created
    # one. This is documentation text only; nothing about the login
    # form/auth logic below it changed.
    st.info(
        "**No default admin account exists.** The deployment owner must "
        "create one using `scripts/create_admin.py` (see supabase/README.md)."
    )
