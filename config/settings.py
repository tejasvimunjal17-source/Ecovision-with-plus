...
# ---- Database ----
# DATABASE_PATH (SQLite) is kept, unused by database/db.py as of Phase 2B,
# purely so nothing that still references settings.DATABASE_PATH elsewhere
# breaks, and as a documented historical fallback path — see
# database/schema.sql's own "kept for reference" note.
DATABASE_PATH = str(BASE_DIR / _get("DATABASE_PATH", "database/ecovision.db"))

# ---- Supabase (PostgreSQL) — Phase 2B ----
# SUPABASE_DB_URL is what database/db.py actually connects with: a direct
# Postgres connection string (Supabase dashboard -> Project Settings ->
# Database -> Connection string -> "URI"), e.g.
#   postgresql://postgres:[password]@db.[project-ref].supabase.co:5432/postgres
# This is required because database/db.py preserves EcoVision's existing
# raw parameterized SQL strings (execute()/fetch_one()/fetch_all() as
# already used throughout backend/, chatbot/, and pages/) — Supabase's
# REST/postgrest API (what SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
# authenticate against) has no way to run arbitrary raw SQL, so it
# cannot be the query path without rewriting every existing query in
# the codebase into postgrest's fluent filter builder. A direct
# Postgres connection (via psycopg2) lets every existing SQL string
# keep working with only the vendor-specific-function fixes documented
# in supabase/README.md ("SQLite -> PostgreSQL translations").
SUPABASE_DB_URL = _get("SUPABASE_DB_URL")

# SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY are captured now for
# completeness and for any FUTURE feature that genuinely needs the
# Supabase REST API or SDK (e.g. Supabase Storage for complaint photos,
# or Supabase Auth in a later phase) — database/db.py does not use
# either of these two in Phase 2B.
...
def is_supabase_configured() -> bool:
    """True once a real (non-placeholder) SUPABASE_DB_URL is present —
    used by database/db.py to fail fast with a clear configuration
    error instead of an obscure connection-string parse error."""
    return bool(SUPABASE_DB_URL) and not _is_placeholder(SUPABASE_DB_URL)
